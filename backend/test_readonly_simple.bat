...
docker exec employee-management-fullstack-app-master-mysql-1 mysql -u third_party_viewer -pReadOnlyAccess2024 employee_management -e "INSERT INTO employees (first_name, last_name, email, age) VALUES ('Test', 'User', 'test@example.com', 25);" 2>nul || echo "INSERT correctly denied"

echo.
echo 4. Testing UPDATE permission (should fail)...
docker exec employee-management-fullstack-app-master-mysql-1 mysql -u third_party_viewer -pReadOnlyAccess2024 employee_management -e "UPDATE employees SET first_name = 'Test' WHERE id = 1;" 2>nul || echo "UPDATE correctly denied"

echo.
echo 5. Testing DELETE permission (should fail)...
docker exec employee-management-fullstack-app-master-mysql-1 mysql -u third_party_viewer -pReadOnlyAccess2024 employee_management -e "DELETE FROM employees WHERE id = 1;" 2>nul || echo "DELETE correctly denied"

echo.
echo 6. Testing access to other tables (should fail)...
docker exec employee-management-fullstack-app-master-mysql-1 mysql -u third_party_viewer -pReadOnlyAccess2024 employee_management -e "SELECT COUNT(*) FROM departments;" 2>nul || echo "Access to departments correctly denied"
